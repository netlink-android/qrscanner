...
class _QrScannerPageState extends State<QrScannerPage> {
  ...
  void scanner() {
    if (data.indexOf('MATMSG') == 0) {
      ...
      print(data.substring(data.indexOf('SUB:') + 4, data.indexOf(';BODY:')));
      print(data.substring(data.indexOf(';BODY:') + 6, data.length - 2));
      dataScanner.add(extractEmailsFromString(data));
      dataScanner.add(
          data.substring(data.indexOf('SUB:') + 4, data.indexOf(';BODY:')));
      dataScanner
          .add(data.substring(data.indexOf(';BODY:') + 6, data.length - 2));
    } else if (data.indexOf('mailto') == 0) {
      print('email');
      typeScanner = 'email';
      dataScanner.add(extractEmailsFromString(data));
      dataScanner.add(
          data.substring(data.indexOf('subject=') + 8, data.indexOf('&body=')));
      dataScanner.add(data.substring(data.indexOf('&body=') + 6));
    } else if (data.indexOf('tel') == 0) {
      print('call');
      typeScanner = 'call';
      dataScanner.add(data.substring(data.indexOf('tel:') + 4));
    } else if (data.indexOf('WIFI') == 0) {
      print('wifi');
      typeScanner = 'wifi';
      if (data.indexOf('WIFI:S:') == 0) {
        dataScanner.add(
            data.substring(data.indexOf('WIFI:S:') + 7, data.indexOf(';T:')));
        dataScanner
            .add(data.substring(data.indexOf(';T:') + 3, data.indexOf(';P:')));
        if (data.indexOf(';H:') > 0) {
          dataScanner.add(
              data.substring(data.indexOf(';P:') + 3, data.indexOf(';H:')));
        } else {
          dataScanner
              .add(data.substring(data.indexOf(';P:') + 3, data.indexOf(';;')));
        }
      } else {
        dataScanner.add(
            data.substring(data.indexOf('WIFI:T:') + 7, data.indexOf(';S:')));
        dataScanner
            .add(data.substring(data.indexOf(';S:') + 3, data.indexOf(';P:')));
        dataScanner
            .add(data.substring(data.indexOf(';P:') + 3, data.indexOf(';H')));
      }
      ...
      final splitLocation = data.substring(data.indexOf('?q=') + 3).split(',');
      // dataScanner
      ...
      dataScanner.add(data.substring(
          data.indexOf('://wa.me/') + 9, data.indexOf('/?text=')));
      dataScanner.add(data.substring(data.indexOf('/?text=') + 7));
      ...
      // ));
    } else if (data.indexOf('https://www.facebook.com') == 0) {
